	...
	return (
		...
		>
			<Col sm="12" md="3" lg="3" className="mb-2">
				<i className="fa fa-cutlery fa-lg" aria-hidden="true">
					{" "}
					Recipes App
				</i>
			</Col>
...
function Footer() {
	return (
		<div
			className="fixed-bottom container bg-dark text-light"
		...
		>
			<p style={{ fontSize: 20 }}>
				...
				<i>
					{" "}
					<a className="text-light" href="https://www.themealdb.com/api.php">
						themealdb.com
					</a>{" "}
				</i>
				&{" "}
				<i>
					{" "}
					<a className="text-light" href="https://github.com/thebhushanmhatre/food-recipe-app">
						Bhushan Mhatre
					</a>
				</i>
			</p>
...
function RenderItem({ item }) {
	...
	const getElements = (item) => {
		...
		nums.forEach((i) => {
			...
			if (element) elements.push(element + " -> " + quantity);
		});
			...
			<CardBody>
				<div className="text-center">
					...
					<Button color="primary" onClick={toggle} style={{ marginTop: 10, marginBottom: 10 }}>
						<i className="fa fa-align-justify fa-lg" aria-hidden="true">
							{" "}
							See Recipe
						</i>
					</Button>
				...
				<Collapse isOpen={isOpen}>
					<span>
						...
						<ul>{getElements(item)}</ul>
					</span>
					<span>
						<u>Instructions:</u> <ol>{getSteps(item)}</ol>
					</span>
				</Collapse>
			</CardBody>
		</Card>
...
function PopularItems(props) {
	...
	return (
		<Alert color="success" style={{ marginTop: 20 }}>
			Some Popular Recipes : {popular_items}
		</Alert>
	);
}
