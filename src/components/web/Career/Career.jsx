...
function Career() {
	...
	const renderContent = () => {
		switch (activeTab) {
			case "born":
				return (
					<div className="text-center pt-5 fade-in" style={{ minHeight: "200px" }}>
						<h2>🌍 Hello World!</h2>
						<p className="text-muted mt-3">Born in 1997</p>
					</div>
				);
			case "school":
				return <div className="pt-3">{schoolData && <RenderInstitute item={schoolData} />}</div>;
			case "patkar":
				return (
					<div className="pt-3">
						{patkarData && <RenderInstitute item={patkarData} imageRight={true} />}
					</div>
				);
			case "vjti":
				return <div className="pt-3">{vjtiData && <RenderInstitute item={vjtiData} />}</div>;
			case "analyst":
				return (
					<CareerCard>
						<CompanyHeader
			...
			default:
				return (
					<div className="text-center pt-5 fade-in" style={{ minHeight: "200px" }}>
						<h2>🚀 Coming Soon...</h2>
						<p className="text-muted mt-3">Something exciting is happening here! 🌟</p>
					</div>
				);
		}
	};
					...
					{careerData.map((step, i) => (
						<React.Fragment key={step.id}>
							...
							{i > 0 && (
								...
								>
									{step.lineText && <div className="timeline-line-text">{step.lineText}</div>}
								</div>
							)}
