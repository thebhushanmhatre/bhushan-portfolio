...
function Project() {
	...
	const getProjects = () => {
		...
		if (selectedFilters.length > 0) {
			...
			return projects.filter((project) => filterValues.every((fil) => project.tech.includes(fil)));
		}
		return projects;
	};
	...
	if (project_list.length > 0) {
		...
		projects = (
			...
			>
				...
				>
					<i
						className={`fa fa-search fa-4x ${darkMode ? "text-info" : "text-primary"}`}
						aria-hidden="true"
					/>
				</div>
				<h2 className="fw-bold mb-3">No matching projects</h2>
				<p
					className={`mx-auto mb-4 fs-5 ${darkMode ? "text-secondary" : "text-muted"}`}
						...
						{selectedFilters.map((f) => (
							<span
								...
								className={`badge rounded-pill px-3 py-2 ${
									darkMode ? "bg-secondary text-light" : "bg-light text-dark border"
								}`}
							>
								{f.label}
							</span>
