...
function Certificate() {
  ...
  const filters = ['Javascript', 'Python', 'Ruby', 'SQL'].map((item) => (
    <Button key={item} className="mr-1" onClick={() => setFilter(item, 'tech')}>
      {item}{' '}
    </Button>
  ));
  ...
  let carouselForm = (
    <span
      onClick={() => toggleCarousel((prevMode) => !prevMode)}
      className={carousel ? 'fa fa-th fa-lg' : 'fas fa-film fa-lg'}
      style={{ float: 'right' }}
    />
  );
  ...
  return (
    <Container>
      ...
      <h3>
        {filterBy ? filterBy : 'My'} Certificates {filters} {carouselForm}
      </h3>
      <h3>{issuers}</h3>
