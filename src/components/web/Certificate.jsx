...
function Certificate() {
	...
	const getCertis = () => {
		...
		if (selectedTech.length > 0) {
			...
			certificates = certificates.filter((item) =>
				techValues.some((tech) => item.tech.includes(tech)),
			);
		}
	...
	if (certList.length > 0) {
		renderTable = <RenderTable className="m-4 pb-4" items={certList} type={"Certificate"} />;
	} else {
		renderTable = <h3 className="text-danger">No Certificates Found</h3>;
	}
